...
class Spielfeld:
    ...
    def possible_felder(self, spieler):
        """
        :param spieler: Entspricht dem aktuellen Spieler 0 steht für "black" 1 für "white"
        :return: Tupel von der Form: (counter, pos_felder). Wobei counter die Anzahl der zulässigen freien Felder und
                 pos_Felder ein 8x8 Array mit den Einträgen "pos_f" falls Feld zulässig und "empty" sonst.
        """
        pos_felder = np.array(["pos_f"] * (8*8)).reshape((8,8))
        return 64, pos_felder
    ...
    def play_game(self, use_bots):
        ...
        if use_bots:
            ...
            # bot_black_0 = my_greedy_bot(spieler_farbe="black")

            bot_white_1 = my_random_bot(spieler_farbe="white")
            # bot_white_1 = my_greedy_bot(spieler_farbe="white")
            ...
            if use_bots == 1 and not self.hat_gewonnen:
                if self.possible_felder(spieler)[0] > 0:
                    if spieler == 0:
                        ...
                        if bot_black_0.cur_choice is None:
                            print("\t\tbot_black_0.cur_choice is None")
                        else:
                            print("\t\tbot_black_0.cur_choice:", bot_black_0.cur_choice)
                    else:
                        ...
                        if bot_white_1.cur_choice is None:
                            print("\t\tbot_white_1.cur_choice is None")
                        else:
                            print("\t\tbot_white_1.cur_choice:", bot_white_1.cur_choice)


                    if idx is None or idy is None:
                        akt_spieler_musste_passen += 1
                        print("self.possible_felder(spieler)[0] =", self.possible_felder(spieler)[0])
...
if __name__ == '__main__':
    ...
    if play_many_games:
        ...
        for elem in range(max_games):
            ...
            for i in range(5):
                print("Das nächste Spiel beginnt in:", 5-i, "Sekunde(n).")
                time.sleep(1)
        print("\n---------------------------------------------")
        ...
        use_bots = 0  # 0 -> two human player, 1 -> two computer player, 2 -> one human and one computer player
        othello = Spielfeld()
